...
A *harness* is the agent process Kiro Crew drives through a provider adapter.
Most current harnesses speak ACP; Codex uses its native app-server protocol.
Kiro Crew has one
first-class harness — `kiro-cli` (`ACP_BACKEND_KIRO`, spelled `""`) — and a
growing set of adapted ones: Codex (`ACP_BACKEND_CODEX`), the dormant
`ACP_BACKEND_CLAUDE` seam, KAS (`ACP_BACKEND_KAS`), and whatever a
...
| H8 | New harness identifiers live in `acp_backends.py` — a LEAF module, so every consumer can name the constants rather than copy them — and are added to `ACP_BACKENDS_KNOWN`; every capability set lives in `acp/types.py` and is a subset of it. `AcpProvider.__init__` accepts only `ACP_BACKENDS_ACP_ADAPTER`, so a known native harness cannot fall through ACP and spawn `kiro-cli` under a foreign label. `ACP_BACKEND_KIRO` is the empty string, so such a fallthrough would launch the default executable under the wrong identity. `acp/types.py` re-exports the identifiers and remains the import site for existing callers. | `test_harness_parity.py::test_capability_sets_are_subsets_of_known_backends`, `::test_unknown_backend_rejected_at_construction`, `::test_native_harness_rejected_by_acp_adapter` | `acp_backends.py` (`ACP_BACKENDS_KNOWN`), `acp/types.py` (`ACP_BACKENDS_ACP_ADAPTER`), `providers/acp.py` (`AcpProvider.__init__`), `scripts/check_harness_parity.py` (`IDENTIFIER_VOCABULARY_PATH`, `CAPABILITY_VOCABULARY_PATH`) |
...
| H11 | The provider label is a closed mapping and an absent label means Kiro. It indexes resume compatibility, session-map persistence, and session-file cleanup routing, so a harness with no `PROVIDER_LABEL_*` of its own persists as a Kiro session. Native adapters expose their label through the ABC rather than being inferred as ACP. | `test_harness_parity.py::test_every_known_backend_has_a_label` | `acp/types.py` (`PROVIDER_LABEL_*`), `providers/base.py` (`provider_label`), `providers/acp.py` (`provider_label`, `cleanup_session`), `session.py` (`detect_provider_switch`) |
