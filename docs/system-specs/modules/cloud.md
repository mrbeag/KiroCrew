...
proxy so kiro-cli can `InvokeGateway` (preferred listen port `18765`,
overridable with `KIROCREW_AGENTCORE_PROXY_PORT`); it does not create
the Gateway or its targets — the operator supplies an existing MCP URL.
...
(`GET`/`PUT /api/agentcore/identity` on that dashboard). The same pane
verifies the Gateway, lists targets and tools, and can Sync a DEFAULT
catalog (`GET`/`POST /api/agentcore/gateway`, owner-only). A hub launching
