...
class AutoNudgeService:
    ...
    def _apply_monitor_user_stop(self, loop: NudgeLoop, *, stopped_at: float) -> None:
        ...
        # The channel marks acceptance synchronously at provider entry, before
        # its outer dispatcher can durably report DISPATCHED. BUSY carries no
        # such runtime marker and must remain restartable.
        accepted = self._accepted_monitor_turns.get(loop.id) == state.last_wake_fingerprint
        if state.wake_delivery is not MonitorDispatchResult.DISPATCHED and not accepted:
            state.wake_in_flight = False
            state.wake_delivery = None
        state.completion_evidence_deadline = 0.0
