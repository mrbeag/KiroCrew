...
def _tool(name: str) -> str | None:
    ...
    # it stays PATH-resolved — but reject a hit inside the agent-writable data
    # home, which is where a planted shim would sit; system node is outside it.
    try:
...
async def _resolve_vetted(u: str, allow_loopback: bool = True) -> list[str] | None:
    ...
    try:
        ...
        infos = await asyncio.to_thread(
            socket.getaddrinfo, p.hostname, port, 0, socket.SOCK_STREAM
        )
    except OSError:
        return None
    ips: list[str] = []
...
async def _discover_from_dir(directory: Path, handle: str) -> dict[str, Any]:
    ...
    cannot_see: list[str] = []
    if routes:
        probe_out = _uploads_dir() / f"dc-probe-{uuid.uuid4().hex[:12]}"
...
async def _handle_discover(request: web.Request) -> web.Response:
    ...
    if kind == "repo":
        ...
        if vetted is None:
            ...
            # loopback (a repo clone has no localhost-preview use).
            return web.json_response(
                {
                    "framework": "",
                    "note": "",
                    "blocked": {"reason": "no-access", "detail": "only http(s) repository URLs to a public host are supported."},
                    "screens": [],
                    "flows": [],
                    "cannotSee": [],
                    "handle": "",
            ...
            return web.json_response(
                {
                    "framework": "",
                    "note": "",
                    "blocked": {"reason": "no-access", "detail": "git is not available on this host."},
                    "screens": [],
                    "flows": [],
                    "cannotSee": [],
                    "handle": "",
        ...
        # Pin git's DNS to the addresses we just vetted. Without this git does its
        # own fresh resolution in a separate process, so a TTL-0 name that answered
        # public to our check could answer 169.254.169.254 / 10.x / loopback to git
        # (DNS rebinding). curloptResolve keeps the hostname (TLS SNI/cert intact)
        # but forces the connection to a vetted IP.
        pu = urlparse(value)
...
async def _handle_render(request: web.Request) -> web.Response:
    ...
    if (len(value) > _MAX_FIELD_LEN or len(handle) > _MAX_FIELD_LEN
            or any(len(r) > _MAX_FIELD_LEN for r in refs)):
        return _bad_request("a field is too long", "field_too_long")

    # A unique per-render output dir: the capture scripts name files by route
    # index, so a shared dir lets a later critique overwrite an earlier one's
    # screenshots and the earlier history then shows the new site.
    out_dir = _uploads_dir() / f"dc-render-{uuid.uuid4().hex[:12]}"
    ...
    try:
        if kind in ("repo", "local"):
            ...
            # Pin the browser's DNS for the base host to a vetted IP so a rebinding
            # name cannot resolve to an internal address on Chromium's own lookup
            # (mirrors the git-clone curloptResolve pin).
            ...
            rc, out, cerr = await _run(cmd, _CAPTURE_TIMEOUT, env=_script_env())
            step = 0
        ...
        # On any no-screen exit (failed capture, expired handle, bad url, empty
        # result) nothing references it and the TTL sweep skips dc-render-*, so drop
        # it here to keep repeated failures from exhausting upload storage.
        if not screens:
