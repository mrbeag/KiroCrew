...
def _build_launcher_script(
    ...
    *,
    ...
    readonly_dirs.append(str(config_dir().resolve() / "docker_registry_access.json"))
...
def _seal_docker_grant_target() -> str:
    """Create an absent disabled grant before Linux seals it read-only.

    Publish without clobbering an operator write. Refuse aliases: a read-only
    bind protects a path, not an aliased inode.
    """
    root = config_dir().resolve()
    root.mkdir(parents=True, exist_ok=True)
    target = root / "docker_registry_access.json"
    if not target.exists() and not target.is_symlink():
        fd, name = tempfile.mkstemp(prefix=".docker-grant-", dir=root)
        try:
    ...
    if not stat.S_ISREG(info.st_mode) or info.st_nlink != 1:
        raise RuntimeError("Docker registry grant must be a regular, unaliased file")
    return str(target)
