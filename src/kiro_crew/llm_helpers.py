...
def fallback_rewound_transient_budget() -> int:
    ...
    surfaces use. Clamped at zero: the counter is also read by the
    pre-stream-failure streak logic, which must never see a negative value.
    """
...
def fallback_story_of(exc: BaseException) -> str:
    ...
    may have refused the set). Redaction and the :data:`_FALLBACK_STORY_CAP`
    ...
    try:
        ...
        logger.debug("fallback story redaction failed", exc_info=True)
        return ""
    return story[:_FALLBACK_STORY_CAP]


def append_fallback_story(text: str, exc: BaseException) -> str:
    ...
    unchanged, byte-for-byte. The story arrives redacted+capped from
    :func:`fallback_story_of`. Never raises.
    """
    story = fallback_story_of(exc)
    if not story:
        return text
    return f"{text} [{story}]" if text else story
...
async def probe_fallback_restore(
    ...
    *,
    ...
    if on_restored is not None:
        ...
        # and the follow-up stale-marker path would clear it anyway.
