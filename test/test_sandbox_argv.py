...
class TestKiroInternalSandboxExclusion:
    ...
    def test_equivalent_default_ledger_is_not_a_custom_home(self, monkeypatch):
        """Filesystem aliases of the default ledger keep Kiro delegation available."""
        ...
        with patch("kiro_crew.sandbox.os.path.samefile", return_value=True) as mock_samefile:
            assert sandbox_mod._run_coordinator_uses_custom_home() is False

        mock_samefile.assert_called_once_with(equivalent_dir, default_dir)

    def test_unidentifiable_ledger_alias_fails_closed(self, tmp_path, monkeypatch):
        """An identity-check failure must not grant Kiro delegation."""
        ...
        with patch("kiro_crew.sandbox.os.path.samefile", side_effect=OSError):
